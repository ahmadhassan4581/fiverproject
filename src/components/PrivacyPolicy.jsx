    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="mb-12 text-center">
          <p className="text-lg text-[#a3a3a3] leading-relaxed max-w-3xl mx-auto">
          ...
          {sections.map((section, index) => {
            ...
            return (
              <div 
                key={section.id}
                id={section.id}
                className="bg-[#1a1a1a] rounded-2xl border border-[#2a2a2a] overflow-hidden group hover:border-[#c6a43f]/50 transition-all"
              >
                {/* Section Header */}
                <div className="flex items-start gap-4 p-6 lg:p-8 border-b border-[#2a2a2a]">
                  <div className="p-3 bg-[#2a2a2a] rounded-xl flex-shrink-0 transition-all duration-300 group-hover:bg-[#c6a43f]/20 group-hover:scale-110">
                ...
                <div className="p-6 lg:p-8 space-y-6">
                    ...
                    <ul className="grid sm:grid-cols-2 gap-3">
                      {section.list.map((item, i) => (
                        <li key={i} className="flex items-start gap-3">
                          <span className="w-2 h-2 bg-[#c6a43f] rounded-full mt-2 flex-shrink-0" />
                  ...
                  {section.note && (
                    <div className="flex items-start gap-3 p-4 bg-[#c6a43f]/10 border border-[#c6a43f]/30 rounded-lg">
                      <AlertCircle className="w-5 h-5 text-[#c6a43f] flex-shrink-0 mt-0.5" />
                      <p className="text-sm text-[#a3a3a3]">{section.note}</p>
                    </div>
                  )}

                  {section.subsections && section.subsections.map((subsection, subIndex) => (
                    <div key={subIndex} className="mt-6 pl-4 border-l-4 border-[#c6a43f]">
                      <h3 className="text-lg font-semibold text-white mb-2">
                        {subsection.title}
                      </h3>
                      {subsection.content && (
                        <p className="text-[#a3a3a3] mb-3">{subsection.content}</p>
                      )}
                      {subsection.list && (
                        <ul className="space-y-2">
                          {subsection.list.map((item, i) => (
                            <li key={i} className="flex items-start gap-2">
                              <span className="w-1.5 h-1.5 bg-[#c6a43f] rounded-full mt-2 flex-shrink-0" />
                              <span className="text-[#a3a3a3]">{item}</span>
                            </li>
                          ))}
                        </ul>
                      ...
                      {subsection.note && (
                        <p className="text-sm text-[#737373] mt-3 italic">{subsection.note}</p>
                      )}
                    </div>
                  ))}
                </div>
        ...
        {/* Contact Section */}
        <div className="mt-16 bg-[#c6a43f]/10 border border-[#c6a43f]/30 rounded-2xl p-8 lg:p-12 text-center">
          ...
          <a 
            href="mailto:Contact@branksa.com"
            className="inline-flex items-center gap-2 px-8 py-4 bg-[#c6a43f] text-[#0a0a0a] font-medium rounded-xl hover:bg-[#c6a43f]/90 transition-all duration-300"
          >
